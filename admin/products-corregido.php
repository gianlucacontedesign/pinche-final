...
require_once '../includes/functions.php'; // Ruta corregida
...
function handleToggleStatus($productId) {
    ...
    if ($result !== false) {
        $status = $result ? 'activado' : 'desactivado';
        return ['success' => true, 'message' => "Producto $status exitosamente"];
    } else {
...
$per_page = 15; // Cambiado a 15 productos por página
...
try {
    ...
    $total = $db->fetchOne($count_query, $params)['total'];
...
<body>
    ...
    <script>
        ...
        function clearFilters() {
            window.location.href = 'products-corregido.php';
        }
